...
export const humanMove = (
  row: string[],
  combination: ICombination[]
) => {
  const stringCombination =
    '' + combination[0].value + combination[1].value;
  const newValue =
    stringCombination === '10' || stringCombination === '00'
      ? '1'
      : '0';
...
export const isGameOver = (row: string[]) =>
  !isMoveLeft(row);
