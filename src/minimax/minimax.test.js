const { isMoveLeft, getWinner, isGameOver } = require('./minimax');
...
test('Return if the computer is winner', () => {
	expect(getWinner(['1', '1'], 'COMPUTER')).toBe('Computer won');
});

test('Return if the player is winner', () => {
	expect(getWinner(['1', '0'], 'COMPUTER')).toBe('Player won!');
});

test('Return if the player is winner', () => {
	expect(getWinner(['1', '1'], 'PLAYER')).toBe('Player won!');
});
